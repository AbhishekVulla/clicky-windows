...
**Status:** Phase 1 + Phase 1.5 complete at 178/178 tests. Path A (12 TDD commits 2026-04-19) + Option B (TTS HTTP double-buffer `4291401`) + Option 2 (STT grace shrink `d29e6dd`) all shipped 2026-04-19/20. Measured: ~1.7s first-audible for multi-sentence (from 5-9s pre-Phase-1.5), ~4.5-6s for single-sentence (first-sentence TTFB floor). Phase 1.5 Step 1 CLOSED earlier — Gemini infra shipped but `.env` defaults to Claude Sonnet 4.6 (Claude 340ms faster + pixel-precise vs Gemini 230px miss). Next: one more cheap latency fix (parallel capture, planned 2026-04-20 but NOT yet coded) → then Phase B starts with B1 installer.
