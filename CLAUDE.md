...
Target end-to-end latency: **~800-1200ms** perceived first-audible-word (aspirational). **Measured post-Path-A + Option B + Option 2** (2026-04-20 debug logs, 178/178 tests green): ~1.7s first-audible-word for multi-sentence Claude responses (sentence streaming fires mid-stream, inter-sentence gaps eliminated by Option B HTTP double-buffer), ~4-6s for single-sentence responses (first-sentence TTFB of ~300ms unchanged — only fixable via Option C WebSocket TTS which is explicitly deferred to post-installer). STT finalize dropped 723ms → ~466ms via Option 2 (grace window 300ms → 100ms). See DECISIONS.md 2026-04-20 entries for Option B (commit `4291401`) + Option 2 (commit `d29e6dd`).
...
Detailed competitor landscape lives in [PRD.md § Competitor Landscape](PRD.md) (refreshed 2026-04-26). Single source of truth — refresh quarterly there, not here. Strategic positioning + writeup pitch + distribution channels are in [LAUNCH.md](LAUNCH.md) (gitignored, internal).

**One-line summary for context:** the trifecta of voice PTT + visual pointer + persistent per-app auto-memory + Windows-native is uncrowded. Closest threat is tekram/clicky-windows (Electron, active, no memory). Memory as a technique alone is no longer novel post-Karpathy-LLM-Wiki / MemPalace virality April 2026.
...
├── tests/              ← 258 tests, mock-based, ~14s full suite
│   ├── test_capture.py
│   ├── test_ai.py
│   ├── test_overlay.py
│   ├── test_stt.py
...
- **Single-instance mutex** (Sprint 3.8, `app.py:761-833` + `app.py:885-901`): `__main__` block acquires Win32 named mutex `Local\\ClickyWindows-SingleInstance-v1` BEFORE `QApplication` construction via `_acquire_single_instance_mutex(kernel32=None)` helper. First instance gets the mutex (returns truthy HANDLE); second sees `ERROR_ALREADY_EXISTS=183`, gets `None`, shows `MessageBoxW` directing user to existing tray icon, and `sys.exit(0)`. **Why:** `pynput.Listener(suppress=False)` is observe-only — multiple processes coexist as independent Win32 `WH_KEYBOARD_LL` hooks, so without this guard one Ctrl+Alt+Space press fired N parallel STT→Claude→TTS pipelines from N processes simultaneously (USER-reported 2026-05-05 — three blue cursor icons stacked in tray, three overlapping voices answering one question). Same canonical pattern Spotify, Slack, Discord, Raycast use. Critical ctypes details: `restype=wintypes.HANDLE` + explicit `argtypes` to prevent x64 HANDLE truncation; `bInitialOwner=False` (existence-as-flag, not ownership); `Local\\` namespace (per-logon-session); `"fail-open"` return string for rare CreateMutexW genuine failure; kernel auto-releases on `ExitProcess` so no shutdown cleanup needed. Trade-off: cannot run two Clickys for testing different MODEL_ID/voice — accept for v0; add `--no-single-instance` flag if needed. See DECISIONS.md 2026-05-05 (Sprint 3.8 entry).
- **Multi-provider TTS dispatch** (Sprint 4, `tts.py` + `app.py` + `settings_dialog.py`): `tts.create_tts_client(provider, api_key)` factory routes case-insensitively `"cartesia"` → `CartesiaSonicTTS`, `"elevenlabs"` → `ElevenLabsTTS`. `app.py:_resolve_tts_credentials()` reads `TTS_PROVIDER` from keyring (via `config.resolve_setting(name, default)` env→keyring sibling to `resolve_api_key` for non-secret config — required per Sprint 3.6 dotenv-trap lesson) and selects the right `*_API_KEY` slot. `ElevenLabsTTS` mirrors `CartesiaSonicTTS` Option B prefetch+playback architecture verbatim with three deliberate divergences: `_generate_response` calls `client.text_to_speech.stream(...)` returning `Iterator[bytes]` directly (no body pre-fetch like Cartesia); `_play_response` converts each int16 PCM chunk to float32 inline (`np.frombuffer(chunk, np.int16).astype(np.float32) / 32768.0`); `stop()` is 5-pronged not 6 because elevenlabs SDK exposes no `response.close()` (break-the-loop via cancel event suffices). Sample rate is per-provider (Cartesia 44100 / ElevenLabs 22050) — each subclass owns its own `sample_rate` + constructs its own `sounddevice.OutputStream` via `_build_player`. Settings dialog dropdown is the single source of truth for provider choice — NO tray submenu (intentional, per USER decision 2026-05-06 to avoid double choice points). `settings_dialog._PROVIDER_CATEGORIES` is a frozen-dataclass tuple driving the 3-row dropdown render (LLM/STT/TTS); Save persists BOTH the provider selection AND the key for that provider's keyring slot. See DECISIONS.md 2026-05-06 (Sprint 4 plan-lock + shipped-note) + `docs/superpowers/plans/2026-05-06-sprint-4-multi-provider.md`.
- **First-audible-word debug log** (Sprint 4 ship-gate, `tts.py` + `app.py`): `tts.arm_first_chunk_callback(cb)` is a one-shot armed callback on the `TTS` abstract base. Both `CartesiaSonicTTS` and `ElevenLabsTTS` hold a `_first_chunk_callback: Callable[[], None] | None = None` slot; in each `_play_response` chunk loop, AFTER the first successful `play(samples)`, fire-and-clear (read slot, set to None, invoke inside try/except so a logging error never breaks audio). `app.py:_pipeline_worker` arms it once per interaction, just before entering the Claude streaming context: `self._tts.arm_first_chunk_callback(lambda: dbg.log("TTS: first audible chunk played"))`. Slot clears after firing → subsequent sentences in the same interaction don't re-fire; next interaction re-arms. **Why:** closes the measurement gap between `CLAUDE: streaming started` (HTTP connection open) and the moment the user actually hears something. Without this, the per-interaction debug log only showed `CLAUDE: done` (total stream end) which user-reads as "13s latency" but the real first-audible-word is ~1.7s for multi-sentence responses (sentence-by-sentence streaming). See DECISIONS.md 2026-05-07 (Sprint 4 ship-gate ADR).
...
- **Memory:** `MemoryStore` has 4 public methods (`__init__`, `recall`, `record`, `list_known_apps`) — NO `infer_skill_level` (removed 2026-04-12 per "not Khan Academy" user pushback). Markdown files + SQLite WAL. `recall()` returns the file tail capped at `MEMORY_RECALL_MAX_CHARS=1500` (config.py:199). Caller (`app.py`) injects into the **user message text content block**, NOT `system=` param. User can `cat EXCEL.EXE.md` — transparency is the UX contract.
...
- **Testing:** ~250+ mock-based pytest tests (258 as of 2026-05-07, was 178 → 182 → 196 → 215 → 216 → 219 → 223 → 255 → 258 across sprints). DI pattern. Full suite green in ~14s. No automated tests for the full screen→AI→overlay→voice loop — manual per-module `py -3.13 -m <module>` gates + demo video for E2E.
