...
```
1. User presses global hotkey (Alt+Space, push-to-talk) — NEVER Ctrl+Space (VS Code IntelliSense conflict)
...
- `pynput.Listener(suppress=True)` for Alt+Space push-to-talk
- `faster-whisper` base model for local STT (CPU, int8 — 4× faster than `openai-whisper`)
- `pyttsx3` for TTS (SAPI, background thread)
...
├── hotkey.py           ← pynput Alt+Space push-to-talk (suppress=True)
...
- **Overlay:** Must be click-through (not steal focus), always-on-top, transparent background, no taskbar entry, spans full virtual desktop. PyQt6's `WA_TransparentForMouseEvents` alone is NOT enough on Windows — you MUST also apply Win32 layered window flags via `ctypes`: `WS_EX_LAYERED | WS_EX_TRANSPARENT | WS_EX_TOPMOST | WS_EX_NOACTIVATE | WS_EX_TOOLWINDOW` after `show()`.
- **Hotkey:** Alt+Space push-to-talk via `pynput.keyboard.Listener(suppress=True)` low-level hook. Fallback: Ctrl+Shift+Space. **NEVER Ctrl+Space** — conflicts with VS Code IntelliSense which would break developer users' autocomplete.
...
numpy            # Audio buffers
faster-whisper   # Local STT, CTranslate2 backend (4× faster than openai-whisper on CPU)
pyttsx3          # Local TTS (Windows SAPI)
