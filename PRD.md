...
**Status:** Phase 1 + Phase 1.5 + parallel-capture refactor + **Sprints 0–3.8 all SHIPPED + USER-verified at 223/223 tests (was 178 → 182 → 196 → 215 → 216 → 219 → 223).** Sprint 3.8 added Win32 named-mutex single-instance guard preventing multi-clicked-shortcut from spawning N parallel Clicky processes — USER manually verified post-install: messagebox fires, second instance exits, tray contains exactly one cursor. **85 MB `Clicky-Windows-Setup-v0.1.0.exe`** at `installer/Output/`. **Sprint 4 plan LOCKED** at `docs/superpowers/plans/2026-05-06-sprint-4-multi-provider.md` (12 TDD tasks: multi-provider settings UX + privacy framing + ElevenLabs TTS opt-in subclass; ~31 net new tests; ~5-6 hours; execution mode = inline `superpowers:executing-plans` with USER checkpoints at end of Tasks 5/9 + before 12). **Next: Sprint 4 execution → Sprint 4.7 doc-sync → Sprint 5/6 README + CI + SignPath + public flip.**
