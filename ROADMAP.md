...
| `docs/superpowers/plans/YYYY-MM-DD-ai.md` | Step 2 (ai.py) | Not started |
...
| 0 | Scaffold (requirements.txt, .env.example, .gitignore, config.py) | ⏳ Pending | `python -c "import anthropic, mss, PyQt6, pynput, sounddevice, numpy, faster_whisper, pyttsx3, PIL, dotenv"` succeeds | — |
| 0.5 | Git init + private GitHub repo | ⏳ Pending | `gh repo view clicky-windows --json visibility` returns "PRIVATE", repo visible in web UI | — |
| 1 | `capture.py` | ⏳ Pending | `python -m capture` saves `debug_capture.jpg` at correct resolution, prints monitor + cursor coords that match real position ±2 px. User mouses over known UI element and confirms. | — |
| 2 | `ai.py` | ⏳ Pending | Load `debug_capture.jpg`, call `AnthropicClient.ask(...)`, ≥1 coordinate parsed inside bounds, response text non-empty. Print token count. | — |
