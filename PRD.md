...
**Status:** Phase 1 + Phase 1.5 + parallel-capture refactor + **Sprints 0–3.6 all SHIPPED at 219/219 tests (was 178 → 182 → 196 → 215 → 216 → 219).** Path A (12 TDD commits 2026-04-19) + Option B (TTS HTTP double-buffer `4291401`) + Option 2 (STT grace shrink `d29e6dd`) shipped 2026-04-19/20. Measured: ~1.7s first-audible for multi-sentence (from 5-9s pre-Phase-1.5), ~4.5-6s for single-sentence. Sprint 0–3.6 (2026-05-04/05): pre-public hygiene + KB upload + PyInstaller installer + tray + first-launch keyring modal + GPT Image v2 icon + bundled-EXE OpenRouter `sk-or-` auto-routing. **84 MB `Clicky-Windows-Setup-v0.1.0.exe` ready** at `installer/Output/`. End-to-end PTT verified working in installed bundle (debug log 2026-05-05_05-16-19). **Next: Sprint 4 (ElevenLabs opt-in TTS, ~1 hr) → Sprint 5 (README + CI + repo metadata + SignPath OSS application + public flip).**
