...
**Status:** Phase 1 + Phase 1.5 + parallel-capture refactor + **Sprints 0–4 + Sprint 4 ship-gate refinements all SHIPPED + USER-verified (258/258 tests, 87 MB `Clicky-Windows-Setup-v0.1.0.exe` ready, end-to-end PTT + single-instance guard + multi-provider TTS UX verified in installed bundle).** Sprint 4 added the 3-category dropdown settings UX (LLM Anthropic / STT AssemblyAI / TTS Cartesia or ElevenLabs) + ElevenLabsTTS opt-in subclass + plain-English privacy line + first-audible-word debug log via one-shot `arm_first_chunk_callback`. Active sprint sequence: ✅ 0–3.6 → ✅ Sprint 3.7 docs → ✅ Sprint 3.8 single-instance mutex → ✅ Sprint 4 multi-provider settings UX + ElevenLabs TTS opt-in → ✅ Sprint 4 ship-gate UX refinements → ⏳ Sprint 4.7 doc-sync (THIS commit) → ⏳ Sprint 5/6 README + CI + repo metadata + SignPath + public flip. See [DECISIONS.md](DECISIONS.md) entries dated 2026-05-04, 2026-05-05, 2026-05-06, and 2026-05-07 for full ADR coverage.
...
| `a1c0a0b` | Sprint 4.0 docs | Pre-Sprint-4 doc-sync batching Sprint 3.7 close-out + Sprint 4 plan-lock ADR + Sprint 3.8 ADR + project_phase1_current_state full rewrite + new feedback memory file for the dotenv-trap lesson |
| `e1d84f9` `798000e` `a96a230` `458e68c` `c481fee` `c5c7b96` `91e501e` `d8c6390` `162d427` `7f6c5eb` `a6d1ecf` | Sprint 4 | Multi-provider settings UX + ElevenLabs TTS opt-in subclass. 11 TDD-task commits + 1 review-feedback commit. Replaces flat 3-required-keys layout with 3-category dropdown UX (LLM/STT/TTS) + privacy line + Get-key buttons. `ElevenLabsTTS(TTS)` mirrors Cartesia Option B prefetch+playback architecture with 3 deliberate divergences (iterator-direct streaming, int16→float32 inline conversion, 5-pronged stop). `tts.create_tts_client(provider, key)` factory dispatches by string. `config.resolve_setting` env→keyring sibling for non-secret config (`TTS_PROVIDER` etc.). 32 net new tests (223 → 255). `/review` (`superpowers:code-reviewer`) cleared 0 T1 issues; T2-1 (stale-provider MessageBox) + T2-3 (dated voice/model verification anchors) applied as `a6d1ecf`. Construction-order Qt-signal hang caught + fixed in-flight at `d8c6390` |
| `3d3f5a0` | Sprint 4 ship-gate | First-audible-word debug log via one-shot `tts.arm_first_chunk_callback` armed by `app.py:_pipeline_worker` per interaction, fires + clears on first `play(samples)` in `_play_response`. Plain-English privacy line ("Nothing leaves your machine." replaces "No server, no telemetry."). USER-driven post-Sprint-4 refinements after manual UX verification + latency-question diagnosis. 3 new tests (255 → 258) |
| `<pending Sprint 4.7 doc-sync>` | Sprint 4.7 | Post-Sprint-4 doc-sync: DECISIONS.md gets closing note on 2026-05-06 ADR + new 2026-05-07 ship-gate ADR. ROADMAP / MEMORY / project_phase1_current_state / CLAUDE / PRD / LAUNCH refreshed for 258/258 tests + 87 MB Setup.exe. CLAUDE.md adds 2 new load-bearing invariants (multi-provider TTS dispatch + first-audible-word debug log) |
